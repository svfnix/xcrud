...
class Xcrud
{
    ...
    public function get_db(){
        return Xcrud_db::get_instance($this->connection);
    }
    ...
    public function columns_config($fields){
        if(is_array($fields)){
            ...
            foreach($fields as $field => $config){
                foreach($config as $key=>$val){
                    switch($key){
                        case 'label':
                            $this->label($field ,$val);
                            break;

                        case 'width':
                            $this->column_width($field ,$val);
                            break;

                        case 'callback':
                            $this->column_callback($field ,$this->serializer->serialize($val));
                            break;
                    }
                }
            }
    ...
    public function before_insert($callable)
    {
        if ($callable)
        {
            $this->before_insert['callable'] = $this->serializer->serialize($callable);
        }
        return $this;
    }
    public function before_update($callable)
    {
        if ($callable)
        {
            $this->before_update['callable'] = $this->serializer->serialize($callable);
        }
        return $this;
    }
    public function before_remove($callable)
    {
        if ($callable)
        {
            $this->before_remove['callable'] = $this->serializer->serialize($callable);
        }
        return $this;
    }
    public function after_insert($callable)
    {
        if ($callable)
        {
            $this->after_insert['callable'] = $this->serializer->serialize($callable);
        }
        return $this;
    }
    public function after_update($callable)
    {
        if ($callable)
        {
            $this->after_update['callable'] = $this->serializer->serialize($callable);
        }
        return $this;
    }
    public function after_remove($callable)
    {
        if ($callable)
        {
            $this->after_remove['callable'] = $this->serializer->serialize($callable);
        }
        return $this;
    }
    public function after_upload($callable)
    {
        if ($callable)
        {
            $this->after_upload['callable'] = $this->serializer->serialize($callable);
        }
        return $this;
    }
    public function before_upload($callable)
    {
        if ($callable)
        {
            $this->before_upload['callable'] = $this->serializer->serialize($callable);
        }
        return $this;
    }
    public function column_callback($fields, $callback = '')
    {
        if(is_array($fields)){
            foreach($fields as $field => $callback) {
                $this->column_callback($field, $callback);
            }
        } else {
            if ($fields && $callback) {
    ...
    public function field_callback($fields, $callback = '')
    {
        if(is_array($fields)){
            foreach($fields as $field => $callback)
            {
    ...
    public function replace_insert($callable)
    {
        if ($callable)
        {
            $this->replace_insert['callable'] = $this->serializer->serialize($callable);
        }
        return $this;
    }
    public function replace_update($callable)
    {
        if ($callable)
        {
            $this->replace_update['callable'] = $this->serializer->serialize($callable);
        }
        return $this;
    }
    public function replace_remove($callable)
    {
        if ($callable)
        {
            $this->replace_remove['callable'] = $this->serializer->serialize($callable);
        }
        return $this;
    }
    public function before_list($callable)
    {
        if ($callable)
        {
            $this->before_list['callable'] = $this->serializer->serialize($callable);
        }
        return $this;
    }
    public function before_create($callable)
    {
        if ($callable)
        {
            $this->before_create['callable'] = $this->serializer->serialize($callable);
        }
        return $this;
    }
    public function before_edit($callable)
    {
        if ($callable)
        {
            $this->before_edit['callable'] = $this->serializer->serialize($callable);
        }
        return $this;
    }
    public function before_view($callable)
    {
        if ($callable)
        {
            $this->before_view['callable'] = $this->serializer->serialize($callable);
        }
        return $this;
    }
    ...
    public function create_action($name, $callable)
    {
        if ($callable && $name)
        {
            $this->action[$name]['callable'] = $this->serializer->serialize($callable);
        }
        return $this;
    }
    ...
    protected function _create($postdata = array())
    {
        ...
        if ($this->before_create)
        {
            ...
            $func = $this->serializer->unserialize($this->before_create['callable']);
            $func($postdata, $this);
    ...
    protected function _entry($mode = 'edit', $postdata = array())
    {
        ...
        {
            $postdata = new Xcrud_postdata($this->result_row, $this);
            $func = $this->serializer->unserialize($this->{$callback_method}['callable']);
            $func($postdata, $this);
            $this->result_row = $postdata->to_array();
        }
    ...
    protected function _remove()
    {
        ...
        {
            $func = $this->serializer->unserialize($this->before_remove['callable']);
            $func($this);
            if ($this->exception)
            {
                $this->task = 'list';
        ...
        {
            $func = $this->serializer->unserialize($this->replace_remove['callable']);
            $this->primary_val = $func($this);
        }
        else
        {
            // remove case
        ...
        {
            $func = $this->serializer->unserialize($this->after_remove['callable']);
            $func($this);
        }
        $this->task = 'list';
        $this->primary_val = null;
    ...
    protected function _save()
    {
        ...
        if (!$this->primary_val)
        {
            ...
            {
                $func = $this->serializer->unserialize($this->before_insert['callable']);
                $func($pd, $this);
            ...
            {
                $func = $this->serializer->unserialize($this->replace_insert['callable']);
                $this->primary_val = $func($pd, $this);
                $postdata = $pd->to_array();
                if ($this->exception)
                {
                    return $this->call_exception($postdata);
            ...
            {
                $func = $this->serializer->unserialize($this->after_insert['callable']);
                $func($pd, $this);
                $postdata = $pd->to_array();
                if ($this->exception)
                {
        ...
        else
        {
            ...
            if ($this->before_update)
            {
                ...
                $func = $this->serializer->unserialize($this->before_update['callable']);
                $x = $func($pd, $row, $this);
            ...
            {
                $func = $this->serializer->unserialize($this->replace_update['callable']);
                $this->primary_val = $func($pd, $this);
                $postdata = $pd->to_array();
                if ($this->exception)
                {
            ...
            {
                $func = $this->serializer->unserialize($this->after_update['callable']);
                $func($pd, $this);
                $postdata = $pd->to_array();
                if ($this->exception)
                {
    ...
    protected function _list()
    {
        ...
        if ($this->before_list)
        {
            $func = $this->serializer->unserialize($this->before_list['callable']);
            $func($this);
        }
        ...
        if ($this->result_row)
        {
            foreach ($this->fields as $field => $fitem)
            {
                ...
                else
                {
                    ...
                    {
                        $func = $this->serializer->unserialize($this->field_callback[$field]['callback']);
                        $this->fields_output[$field] = array(
                            'label' => $this->fields_names[$field],
                            'field' => $func($field, $mode, $this),
                            'name' => $field,
                    ...
                    {
                        $func = $this->serializer->unserialize($this->column_callback[$field]['callback']);
                        $this->fields_output[$field] = array(
                            'label' => $this->fields_names[$field],
                            'field' => $func($field, $this),
                            'name' => $field,
                            'value' => $this->result_row[$field]
                        );
                    }
    ...
    protected function _upload_file()
    {
        ...
        if (isset($_FILES) && isset($_FILES['xcrud-attach']) && !$_FILES['xcrud-attach']['error'])
        {
            ...
            {
                $func = $this->serializer->unserialize($this->before_upload['callable']);
                $func($field, $filename, $this);
                if ($this->exception)
                {
    ...
    protected function _render_list_item($field, $value, $primary_val, $row)
    {
        ...
        if (isset($this->column_callback[$field]) && $row)
        {
            $func = $this->serializer->unserialize($this->column_callback[$field]['callback']);
            $value = $func($value, $field, $primary_val, $row, $this);
            return $value;
        }
    ...
    protected function _render_export_item($field, $value, $primary_val, $row)
    {
        ...
        {
            $func = $this->serializer->unserialize($this->column_callback[$field]['callback']);
            $value = strip_tags($func($value, $field, $primary_val, $row, $this));
            return $value;
        }
    ...
    protected function file_link($field, $primary_val, $thumb = false, $crop = false, $filename = false)
    {
        ...
        return Xcrud_config::$ajax_uri . '?' . http_build_query($params);
    }
    ...
    protected function render_search()
    {
        ...
        if ($this->is_search)
        {
            ...
            $attr = array(
                'class' => 'xcrud-action margin-right-5px',
                'href' => 'javascript:;',
                'data-search' => 1);
    ...
    protected function save_file_to_tmp($file, $filename, $field)
    {
        ...
        {
            $func = $this->serializer->unserialize($this->after_upload['callable']);
            $func($field, $filename, $file_path, $this);
        }
    ...
    protected function remove_tmp_image($filename, $field)
    {
        ...
        {
            $func = $this->serializer->unserialize($this->after_resize['callable']);
            $func($field, $filename, $this);
        }
    }
    ...
    protected function render_table_name($mode = 'list', $tag = 'h2', $to_show = false)
    {
        ...
        if ($this->is_title)
        {
            ...
            {
                case 'create':
                    $out .= $this->is_rtl ? $this->table_name . '<small> - ' . $this->lang('add') . '</small>' : '<small>' . $this->lang('add') . ' - </small>' . $this->table_name;
                    break;
                case 'edit':
                    $out .= $this->is_rtl ? $this->table_name . '<small> - ' . $this->lang('edit') . '</small>' : '<small>' . $this->lang('edit') . ' - </small>' . $this->table_name;
                    break;
                case 'view':
                    $out .= $this->is_rtl ? $this->table_name . '<small> - ' . $this->lang('view') . '</small>' : '<small>' . $this->lang('view') . ' - </small>' . $this->table_name;
                    break;
                default:
    ...
    protected function _call_action()
    {
        ...
        {
            $func = $this->serializer->unserialize($this->action[$name]['callable']);
            $func($this);
        }
        $this->task = $this->after;
        $this->after = null;
